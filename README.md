...
# Ducktape
An open source C++ 2D & 3D game engine that focuses on being fast, and powerful, using Vulkan for rendering. As the engine is currently work in progress, it only supports an ECS. Rendering powered by Vulkan, Audio powered by OpenAL, Physics powered by PhysX are yet to be made.

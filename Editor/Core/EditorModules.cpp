...
namespace DT
{
    ...
    void EditorModules::ResourceBrowser(Engine *engine)
    {
        ...
        fs::path relativePath = fs::relative(currentDir,rootDir);
        fs::path tempDir = rootDir;
        fs::path::iterator it = relativePath.begin();
        ...
        if(ImGui::BeginListBox("##filesListBox",ImVec2(-FLT_MIN,0)))
        {
            ...
            if(Filter.IsActive())
            {
                for (auto& directoryEntry : fs::recursive_directory_iterator(rootDir))
                {
            ...
            {
                for (auto& directoryEntry : fs::directory_iterator(currentDir))
                {
                    DrawDirectoryItem(engine,directoryEntry,itemSize,i,selectedIndex);    
                    i++;
                }
    ...
    void EditorModules::OnItemDoubleClicked(bool isDir,fs::path path)
    {
        if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
		{
			if(isDir)
            {
                currentDir = rootDir / fs::relative(path,rootDir); 
                Filter.Clear();
            }
            else 
#ifdef _WIN32
